...
int main() {
    ...
    map.tileIds = static_cast<unsigned char*>(std::calloc(map.tilesX * map.tilesY, sizeof(unsigned char)));
    map.tileFog = static_cast<unsigned char*>(std::calloc(map.tilesX * map.tilesY, sizeof(unsigned char)));
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        playerTileX = static_cast<int>((playerPosition.x + MAP_TILE_SIZE / 2) / MAP_TILE_SIZE);
        playerTileY = static_cast<int>((playerPosition.y + MAP_TILE_SIZE / 2) / MAP_TILE_SIZE);
        ...
        // Draw player
        DrawRectangleV(playerPosition, Vector2{ static_cast<float>(PLAYER_SIZE), static_cast<float>(PLAYER_SIZE) }, RED);

        // Draw fog of war (scaled to full map, bilinear filtering)
        DrawTexturePro(fogOfWar.texture, Rectangle{ 0, 0, static_cast<float>(fogOfWar.texture.width), static_cast<float>(-fogOfWar.texture.height) },
                       Rectangle{ 0, 0, static_cast<float>(map.tilesX * MAP_TILE_SIZE), static_cast<float>(map.tilesY * MAP_TILE_SIZE) },
